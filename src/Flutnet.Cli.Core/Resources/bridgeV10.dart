...
class _PlatformChannel {
  ...
  final _platformChannel = MethodChannel('flutterbridge.appchannel.incoming');

  // Native channel used to verify if flutter is embedded or not
  final _testChannel = MethodChannel('flutterbridge.appchannel.test');

  Future<bool> _isAppEmbedded() async {
    try {
      var value = await _testChannel.invokeMethod("test", "test");
      return true;
    } catch (ex) {
      return false;
    }
      ...
      await _sendLock.synchronized(() {
        if (_sendRequestMap.containsKey(msg.methodInfo.requestId)) {
          ...
          if (msg.exception != null) {
            var exception = FlutterException.fromJsonDynamic(
        ...
        try {
          ...
          if (isAppEmbedded) {
            ...
            completer.completeError(Exception(
              "The Bridge is configured to work in mode ${BridgeMode.WebSocket}, but you are trying to use the ${BridgeMode.PlatformChannel} communication.\n"
              "Run Flutter in you IDE, like Visual Studio Code.",
            ));
          } else {
            // The user have run flutter using visual studio code, but the configuration cannot be BridgeMode.PlatformChannel
            completer.completeError(Exception(
              "Flutter module is configured to use the ${BridgeMode.PlatformChannel} communication, but you are trying to run it in your IDE (like Visual Studio Code).\n"
              "This is an invalid configuration. Change the Brigde mode in flutter mudule to ${BridgeMode.WebSocket}.",
            ));
          }
        } on Exception catch (ex) {
...
class _DebugChannel {
  ...
  // Native channel used to verify if flutter is embedded or not
  final _platformChannel = MethodChannel('flutterbridge.appchannel.test');

  Future<bool> _isAppEmbedded() async {
    try {
      var value = await _platformChannel.invokeMethod("test", "test");
      return true;
    } catch (ex) {
      return false;
    }
    ...
    _sendLock.synchronized(
      () async {
        ...
        if (isAppEmbedded) {
          // Invalid call
          completer.completeError(Exception(
            "The Bridge is configured to work in mode ${BridgeMode.WebSocket}, but this module can only work using ${BridgeMode.PlatformChannel}\n"
            "Rebuild the flutter module with the right Bridge configuration: mode ${BridgeMode.WebSocket}.",
          ));
          return;
        }
